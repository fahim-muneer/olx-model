...
import { googleLogin, logoutUser } from "../../Redux/AuthSlice"
...
export default function Login() {
  ...
  return (
    ...
    show={status} onClose={() => dispatch(close()) } >


        <div onClick={(e)=>{e.stopPropagation()}} className="p-6 pl-2 pr-2 bg-white">
            <img  onClick={() => dispatch(close()) } className="w-6 absolute z-10 top-4 right-4 cursor-pointer" src={clos} alt="" />
        ...
        <ModalBody className="bg-white h-96 p-0 rounded-none" onClick={(event)=> {event.stopPropagation()}} >
            <div className="p-6 pt-0">
                                    ...
                                    <div onClick={()=>dispatch(googleLogin())}  className="flex items-center justify-center rounded-md border-2 border-solid border-gray-300 p-5 relative h-8 cursor-pointer active:bg-teal-100"  >
